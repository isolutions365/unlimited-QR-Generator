...
export function auditAllRoutes(locale: Locale): { routes: AuditedRouteInfo[]; summary: SEOAuditSummary } {
  const routeList: { path: string; category: AuditedRouteInfo['category']; defaultTitle: string; defaultDesc: string; customType?: string }[] = [
    ...
    { path: '/wifi-qr-generator', category: 'Generators & Tools', defaultTitle: 'Free WiFi QR Code Generator | Connect Without Typing the Password', defaultDesc: 'Generate customized WiFi QR codes with our free WiFi QR code generator. Allow guests to scan and connect without typing the password.' },
    ...
    if (!existingSchemas.includes('Organization') && item.path !== '/' && item.path !== '/about') {
      missingSchemas.push({
        ...
        sampleSnippet: {
          ...
          'name': 'FreeQRBarcodes',
          'url': ROOT_DOMAIN,
          'logo': `${ROOT_DOMAIN}/apple-touch-icon.png`,
          'sameAs': [
