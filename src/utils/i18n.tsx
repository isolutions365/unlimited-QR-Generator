...
import { EXPECTED_KEYS, LocaleReport, generateFullReport, ValidationReport } from './i18nValidator';
import { i18n, loadI18nLocale } from '../i18n';
import en from '../locales/en.json';

const enDictionary: Record<string, string> = en;

// Explicit dynamic chunk loader map for code splitting
const localeLoaders: Record<Locale, () => Promise<{ default: Record<string, string> }>> = {
...
// Active in-flight requests map to prevent duplicate concurrent network fetches
const pendingLoads = new Map<Locale, Promise<Record<string, string>>>();

export async function fetchLocaleDictionary(loc: Locale): Promise<Record<string, string>> {
  if (dictionaryCache[loc]) {
    return dictionaryCache[loc];
  }
  if (pendingLoads.has(loc)) {
    return pendingLoads.get(loc)!;
  }
  ...
  const loadPromise = loader()
    .then((mod) => {
      ...
      pendingLoads.delete(loc);
      return dict;
    })
    .catch((err) => {
      pendingLoads.delete(loc);
      console.warn(`[i18n] Error dynamically loading locale dictionary "${loc}":`, err);
      return dictionaryCache['en'] || enDictionary;
    });

  pendingLoads.set(loc, loadPromise);
  return loadPromise;
}
...
export function I18nProvider({ children }: { children: React.ReactNode }) {
  ...
  const [dictionary, setDictionary] = useState<Record<string, string>>(() => {
    return dictionaryCache[locale] || enDictionary;
  });
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    fetchLocaleDictionary(locale)
      .then((dict) => {
        if (isSubscribed && currentRequestId === latestRequestIdRef.current) {
          setDictionary(dict);
          setIsLoading(false);
        }
      })
  ...
  /**
   * Loads all supported dictionaries on demand.
   */
  const loadAllDictionariesForAnalysis = async (): Promise<Record<Locale, Record<string, string>>> => {
    const entries = await Promise.all(
      (SUPPORTED_LOCALES as Locale[]).map(async (loc) => [loc, await fetchLocaleDictionary(loc)] as const)
    );
...
export function useTranslation(): I18nContextType {
  ...
  if (context === undefined) {
    return {
      ...
      loadAllDictionariesForAnalysis: async () => ({ en: enDictionary }) as any,
