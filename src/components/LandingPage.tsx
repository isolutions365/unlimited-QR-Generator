...
import StaticVsDynamicComparison from './StaticVsDynamicComparison';
...
export const LandingPage: React.FC<LandingPageProps> = ({
  ...
  const handleChooseStatic = () => {
    if (onSelectStatic) {
      onSelectStatic();
    } else if (onPrimaryCTA) {
      onPrimaryCTA();
    } else {
      handleLaunchStudio();
    }
  ...
  return (
    <div className="relative w-full bg-slate-50 text-slate-900 overflow-hidden border-b border-slate-200/80 pt-4 pb-8 lg:pt-8 lg:pb-16 shadow-xs z-0">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        {/* Dedicated, Transparent Static vs Dynamic Comparison Section */}
        <StaticVsDynamicComparison
          onSelectStatic={handleChooseStatic}
          onSelectDynamic={handleChooseDynamic}
        />
      </div>
    </div>
  );
