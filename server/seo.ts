...
import { getAllVerifiedRoutes } from '../src/data/sitemapRegistry';
...
export const sitemapRoutes: SitemapRoute[] = getAllVerifiedRoutes().map(r => ({
  path: r.path,
...
export const landingPageFaqs: Record<string, Array<{q: string; a: string}>> = {
  ...
  'wifi-qr-generator': [
    ...
    { q: 'Which WiFi security options are supported?', a: 'The generator supports the standard T:WPA schema recognized for WPA and WPA2 Personal networks (with transition-mode compatibility on supported devices, but not guaranteeing pure WPA3-SAE-only networks), legacy WEP (T:WEP), and Open unencrypted networks (T:nopass).' },
...
export function buildSitemapXml(): string {
  const baseUrl = "https://www.freeqrbarcodes.com";
  const allRoutes = getAllVerifiedRoutes();
  ...
  console.info(`Sitemap generation validated: successfully compiled exactly ${count} verified canonical URLs.`);
